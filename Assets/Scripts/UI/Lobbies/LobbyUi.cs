...
public class LobbyUi : MonoBehaviour, ILobbyListener
{
    ...
    {
        JoinedLobby.GetLobbyRoomAccess(OnRoomAccessReceived, Debug.Log);
    }
    ...
    {
        JoinedLobby.StartGame(() => { }, Debug.Log);
    }
    ...
    {
        PlayerCount.text = $"{Users.Count} Players ({JoinedLobby.Data.MaxPlayers} Max)";
    }
    ...
    {
        UnityMainThreadDispatcher.Instance().Enqueue(() => { GameStatus.text = statusText; });
    }
